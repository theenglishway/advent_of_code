defmodule AdventOfCode.Day5.Grid do
  ...
  def new(list_of_vent_lines) when is_list(list_of_vent_lines) do
    {_, {max_x, _}, _} =
      list_of_vent_lines
      |> Enum.max_by(fn {:vent_line, {start_x, _}, {end_x, _}} ->
        max(start_x, end_x)
      end)

    {_, {_, max_y}, _} =
      list_of_vent_lines
      |> Enum.max_by(fn {:vent_line, {_, start_y}, {_, end_y}} ->
        max(start_y, end_y)
      end)
  ...
  defp add_point_coverage(current, point), do: current |> Map.update(point, 1, &(&1 + 1))

  def number_of_points_with_coverage_over(%{points_coverage: coverage}, n),
    do: coverage |> Map.values() |> Enum.count(&(&1 >= n))
end

defmodule AdventOfCode.Day5.VentLine do
  def is_vertical?({:vent_line, {x1, _}, {x2, _}}), do: x1 == x2
  def is_horizontal?({:vent_line, {_, y1}, {_, y2}}), do: y1 == y2
  def is_diagonal?({:vent_line, {x1, y1}, {x2, y2}}), do: abs(x2 - x1) == abs(y2 - y1)
  def is_straight?(line), do: is_vertical?(line) or is_horizontal?(line)

  def points_covered(line = {:vent_line, {x1, y1}, {x2, y2}}) do
    cond do
      ...
      true -> raise "Can not get coverage for non-straight or non-diagonal line"
    end
  end
end
  ...
  def into_vents_lines(string) when is_binary(string) do
    ...
    |> Enum.map(fn line ->
      with [start_, end_] <- line |> String.trim() |> String.split(" -> "),
           ...
           do: {:vent_line, {start_x, start_y}, {end_x, end_y}}
    end)
  end
